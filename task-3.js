...
//     const filteredArray = array.filter(function (arr) { return arr.gender === gender })
//     console.log(filteredArray); //возвращает все объекты пола "male"
//     return filteredArray.map(function (arr) { return arr.name })
// };

 //---------РЕШЕНИЕ ЧЕРЕЗ ОБЫЧНУЮ ФУНКЦИЮ C ДЕСТРУКТУРИРУЮЩИМ ПРИСВАИВАНИЕМ----------
const getUsersWithGender = function (array, gender) {
    const filteredArray = array.filter(function (arr) { return arr.gender === gender })
    return filteredArray.map(function ({name}) { return name })
};

//---------РЕШЕНИЕ ЧЕРЕЗ СТРЕЛОЧНУЮ ФУНКЦИЮ -----------
// const getUsersWithGender = (array, gender) => {
//         return array.filter(arr => arr.gender === gender).map(arr => arr.name);
//     };
